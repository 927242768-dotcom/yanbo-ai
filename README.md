...
- 兼容训练累计：正式选择 700 步（本轮尝试到 750 步，因真实任务退化未发布）
- Git 主分支：`main`
...
- 已完成 3 个训练轮次
- 正式适配器累计 700 步；本轮独立候选尝试到 750 步，但因真实任务得分回落而拒绝发布
- 第3轮专项验证损失：`2.3966 → 2.2848`（与旧综合验证集的 `0.8843` 口径不同，不能直接横向比较）
- 综合训练集 2082 条、综合验证集 206 条
- 专项攻坚训练集 548 条、专项验证集 11 条
- 纠错强化训练集 1680 条、纠错验证集 10 条
- 训练断点：`adapters\yanbo-v3-compat-lora`
- 旧正式适配器自动备份在 `adapters\backups\`

兼容训练主要用于表达风格、固定格式、小范围能力补强和正式服务不可用时的回退。日常运行的快速、思考和专家模式始终使用统一的 `yanbo-v3:latest`；本轮新增的确定性能力层会同时服务三种模式。
...
```text
训练数据审计：零重复样本、零空样本、零训练/验证泄漏
原始兼容模型专项得分：7/18 → 8/18
正式引擎专项得分：7/18 → 18/18
严格句数、比例、平均数、真实性、因果判断：通过
Python可变默认参数、C缓冲区、Git取消暂存、SQL空组统计：通过
OCR关键歧义、FPGA跨时钟域、测试边界：通过
750步候选：真实任务回落到7/18，已拒绝发布
身份统一检查：通过
```
