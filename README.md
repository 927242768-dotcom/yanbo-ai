...
- 兼容训练累计：正式适配器保持700步；第5轮两个隔离候选均训练到720步，但独立裸模型评测不足以证明优于正式方案，因此拒绝覆盖
- 正式引擎第4轮主泛化评测：`21/48 → 48/48`；第4轮发布留出集：`28/28`
- 第5轮正式原生引擎评测：`52/60 → 60/60`；第5轮发布留出集：`28/28`
- 当前确定性能力回归：`39/39`；第5轮鲁棒性兼容后端回归：`60/60`；第4轮旧发布留出集兼容后端回归：`28/28`
- Git 主分支：`main`
...
| `round5_tools.py` | 高可靠数学、电子、工程、工具和证据边界能力 |
...
- 已完成5个训练与能力升级轮次
- 正式适配器累计700步，训练断点仍为 `adapters\yanbo-v3-compat-lora`
- 第5轮通用课程：训练集2638条、验证集15条
- 第5轮失败聚焦课程：训练集1411条、验证集15条
- 两套数据均为零完整样本重复、零空样本、零不可用答案、零代码围栏错误、零训练/验证泄漏
- 通用候选从700步训练到720步，验证损失 `1.5374 → 1.5172`
- 失败聚焦候选从700步训练到720步，验证损失 `1.5374 → 1.5190`
- 两个720步候选的独立裸模型评测均为 `20/60`，不覆盖正式700步适配器
- 正式原生引擎通过格式契约、证据边界、电子计算和确定性能力链，由 `52/60` 提升到 `60/60`
- 第5轮全新发布留出集结果为 `28/28`
- “拒绝伪造文献”已与数据手册、页码和厂商公告路由分离，双向防串线回归通过
- 当前确定性能力回归为 `39/39`，第5轮鲁棒性兼容后端回归为 `60/60`，第4轮旧发布留出集兼容后端回归为 `28/28`
- 旧正式适配器备份继续保存在 `adapters\backups\`

兼容训练主要用于表达风格、固定格式、小范围能力补强和正式服务不可用时的回退。日常运行的快速、思考和专家模式始终使用统一的 `yanbo-v3:latest`。完整过程见 `training_reports/ROUND4_TRAINING_REPORT_2026-07-25.md` 和 `training_reports/ROUND5_TRAINING_REPORT_2026-07-27.md`。
...
python evaluate_round5_deterministic.py
python evaluate_round5_robustness.py --execution engine --engine-backend native
python evaluate_round5_release_holdout.py
```
...
第4轮与第5轮训练、评测和正式引擎升级结果：

```text
第4轮正式运行引擎：21/48 → 48/48
第4轮发布留出集：28/28
第5轮通用课程：2638条训练数据；15条验证数据
第5轮失败聚焦课程：1411条训练数据；15条验证数据
第5轮数据审计：零完整样本重复、零空样本、零不可用答案、零代码围栏错误、零训练/验证泄漏
第5轮通用LoRA候选：720步，裸模型20/60，拒绝覆盖正式版本
第5轮失败聚焦LoRA候选：720步，裸模型20/60，拒绝覆盖正式版本
第5轮正式原生引擎：52/60 → 60/60
第5轮发布留出集：28/28
当前确定性回归：39/39
第5轮鲁棒性兼容后端回归：60/60
第4轮旧发布留出集兼容后端回归：28/28
旧18题专项：18/18
模型升级完整评测：10/10
完整基础与代码质量：12/12
数学工具：7/7
